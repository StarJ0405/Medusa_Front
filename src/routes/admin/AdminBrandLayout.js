...
import Node from "./vendor/tree/Node";
...
function AdminBrandLayout() {
    const [initData, setInitData] = useState();
    const [activeNode, setActiveNode] = useState();
    const [collapsed, setCollapsed] = useState(false);
    const [tree, setTree] = useState({ id: "root", titleKr: "브랜드", titleCn: "categpry", type: "CATEGORY", leaf: false, children: [] });
        ...
        if (beforeTreeState === undefined) {
            ...
            if (isChanged) {
                ...
                if (origin.length === changed.length) {
                    //update
                    if (difference) {
                        if (difference.type === ProductType.BRAND) {
                            requester.updateBrand(data, (result) => {
                                if (result.code === 0) {
                        ...
                        } else if (difference.type === ProductType.PRODUCT) {
        ...
        <div className={style.container}>
            {
                ...
                <HorizontalFlex gap={50} height={"100%"}>
                    <FlexChild width={300} height={"100%"} >
                        <div className={style.tree}>
                            <Tree paddingLeft={20} tree={tree}
                                onChange={onTreeUiChange}
                                renderNode={(node) => <Node root={tree.id} type={ProductType.BRAND} node={node} activeNode={activeNode} setActiveNode={setActiveNode} handleContextClick={handleContextClick} addItem={addItem} />}
                            />
                        </div>
                    </FlexChild>
                    <FlexChild height={"100%"} overflowY={"scroll"} backgroundColor={"white"}>
