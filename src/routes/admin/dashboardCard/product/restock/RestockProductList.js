...
import style from "./RestockProductList.module.scss";
...
function RestockProductList() {
    ...
    useEffect(() => {
        requester.getRestockProductsForAdmin((result) => {
            if (result.data) {
    ...
    const onSaveClick = () => {
        ...
        requester.saveRestockProducts(data, (result) => {
            if (result.data) {
                let onlyId = result.data.map((product) => product.productId);
                requester.getAllBrandsForAdmin((result) => {
                    let brandList = result.data;
