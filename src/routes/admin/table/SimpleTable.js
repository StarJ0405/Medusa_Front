...
function SimpleTable(props) {
  ...
  const onCheckGroupCallback = (checkedIndex, value) => {
    ...
    checkedIndex &&
      checkedIndex.map((selectedIndex, index) => {
            ...
            <HorizontalFlex gap={20}>
              <FlexChild width={"max-content"}>
                <Center width={"100%"} textAlign={"left"}>
                  <Inline>
                    <P>검색결과 : </P>
                    <P weight={"bold"} color={"var(--admin-main-color)"}>
                      {data?.length}
                    </P>
                    <P>건</P>
                  </Inline>
...
const SimpleTableRow = forwardRef((props, ref) => {
  ...
  const onModifyClick = () => {
    ...
    NiceModal.show("adminPurchaseOrder", {
      data: data,
          ...
          header.map((col, index) => {
            ...
            if (col.text === "상품명" && Array.isArray(cellData)) {
              displayValue = cellData
                ?.map(
                  (item) => `${item.product.brand.title} ${item.product.title}`
                )
                .join("\n");
            ...
            // Check if we're in the '수량' column and handle accordingly
              ...
              return (
                <FlexChild width={70}>
                  {data?.status && data?.status !== "Canceled" ? (
                    <VerticalFlex>
                      <FlexChild>
                        <CustomButton
                          text={"수정"}
